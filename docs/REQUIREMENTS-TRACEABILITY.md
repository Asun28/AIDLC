...
| MS2 handoff reconciles old effects first | `src/coordination/lease.ts` (`takeover`), `src/coordination/reconcile.ts` (`unresolved`), CLI `goal takeover`, `src/loop/card-runner.ts` (`takeover`), CLI `card takeover` | `tests/infra/lease.test.ts`, `tests/scenarios/two-windows.test.ts` (T0-CARD-TAKEOVER, T0-CARD-TAKEOVER-2) | implemented to the store's limits: no compare-and-set, the remaining windows named in `docs/OPERATIONS.md` (Sessions) |
...
| MS4 quota/congestion is WAIT with one notification owner | `src/coordination/review-queue.ts` (`hold`), `src/core/parse-guard.ts` (`detectQuotaHold`), `src/loop/card-runner.ts` | `tests/infra/review-queue.test.ts`, `tests/core/parse-guard.test.ts` | implemented |
...
| Q23 shared sessions: one writer per card, takeover reconciles first | `src/coordination/lease.ts`, `src/core/card-machine.ts`, `src/loop/card-runner.ts` (`takeover`) | `tests/infra/lease.test.ts`, `tests/core/card-machine.test.ts`, `tests/scenarios/two-windows.test.ts` | implemented in-process; two real windows on one repo need a live qualification run |
