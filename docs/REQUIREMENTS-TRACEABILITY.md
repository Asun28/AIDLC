...
| MS2 handoff reconciles old effects first | `src/coordination/lease.ts` (`takeover`), `src/coordination/reconcile.ts` (`unresolved`), CLI `goal takeover`, `src/loop/card-runner.ts` (`takeover`), CLI `card takeover` | `tests/infra/lease.test.ts`, `tests/scenarios/two-windows.test.ts` (T0-CARD-TAKEOVER) | implemented |
