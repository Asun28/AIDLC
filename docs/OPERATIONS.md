...
- `aidlc goal extend <id> --until <iso> --by <who> --reason "..."` is the only way to move a deadline; retries, revisions and delayed approvals never extend it. An extension (an ISO-8601 UTC timestamp) re-admits a goal stopped for time and every card of it stopped for time (their deadlines move to the new goal deadline, the runs become selectable again; a stop for any other reason stays); re-entry runs through the projection check, so a T2 goal is re-authorized before any dispatch. `aidlc goal resume <id> --reason "..." [--text "..."] [--cards a,b] [--replace '{"old":"new"}']` carries a revision into the resume (replacement ids must exist in the registry) and applies it before the projection, re-entering through CARDS with the completion evidence of the old projection invalidated, the only way to re-plan a terminal goal (a DONE goal included): `aidlc goal amend` refuses a terminal goal and names the resume.
