...
Same-candidate rule: `review pre` and `review r3` refuse a candidate that the same stage already blocked while any finding of that block is open (the refusal names the ids and the two moves: repair to a new candidate, or dispute each finding), and run on the unchanged candidate once every finding of the block is disputed; a block that recorded no cited finding is never answered that way. The formal stage keys this on the candidate's own last decision (`ReviewInvocation.mergeBlocking` and the findings raised or re-raised on that sha), never on the run's last verdict or the decision counter. `aidlc card next` follows the same rule: a pending block with open findings returns the repair directive naming them; with every finding disputed the SHIP gate issues the next round or decision on the unchanged candidate, reusing the DoD receipt the block cleared (`CardRun.blockedReceipt`, bound to the candidate digest) so no further attempt is recorded for an unchanged candidate; the same reuse lets exhausted R2 rounds decide at once. Disputes reach a reviewer only through the command path: with `formalReview.command` empty the ship-path reviewer re-reads a verdict file, so a disputed formal block stays pending until the candidate changes and the dispute is recorded for the human adjudicator. A pre-review pass stays valid for the candidate it reviewed in any later cycle, so a disputed R3 block on an unchanged candidate goes straight to decision two. Dispositions act on the persisted run under the card-run lock (`GoalStore.updateCardRun`: an exclusive `.lock` file next to the record, a 2 s wait, a 30 s stale age for a crashed writer, and a takeover serialized by a second exclusive marker so a live lock is never removed), never on a caller's snapshot: the record is read inside the lock, a stop saved meanwhile refuses, the lease is fenced, and only the changed finding is written back, so one disposition never overwrites another. A routed skip decides nothing about the findings. A dispute needs a note; one dispute per re-raise, a withdrawn one included, so a second dispute of the same finding needs a re-raise in between; a finding re-raised after a dispute twice (two rounds of mutual non-acceptance) is a deadlock: a third dispute is refused, the STOP detail of the R3 second block or of exhausted R2 rounds names it, and with `preReview.onExhausted: "ship"` the residual handed to R3 names it in the journal (`PRE_REVIEW_DECIDED` with `deadlock`) and in the R3 prompt, where the finding's prior-findings line carries the deadlock marker. The human ruling path is the existing STOP/review hand-off. `PRE_REVIEW_DECIDED` and `REVIEW_DECIDED` carry the `findings`, `reraised` and `resolved` id lists (empty on the exhausted and discarded events, which add `residualFindings` for the hand-off); `FINDING_DISPUTED` and `FINDING_ACCEPTED` record the dispositions.
