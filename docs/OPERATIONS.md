...
| `verify-before-done` | Stop | an active card run in BUILD, SHIP or REVIEW_FIX has no DoD receipt and its card lease is absent, released or owned by the acting session (the hook event's `session_id`, or `AIDLC_SESSION`); a run whose lease names another session is that session's to verify; adds Stop context asking for the verification output |
