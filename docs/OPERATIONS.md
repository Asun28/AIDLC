...
`command` is argv (the prompt arrives on stdin; on Windows it runs through a shell unless `shell` is set); an empty command disables the stage. `aidlc review pre <card>` builds the prompt from `REVIEW.md`, the card (acceptance, allow_paths, non_goals, forbid, tier, diagnosis), the committed diff against the base and the run's unresolved findings with their dispositions (see Findings and dispositions), runs the command with a receipt, takes the last JSON-looking line as the verdict (that line decides alone, by a string-aware brace walk over its top-level documents: a document cut short, or a second one started and not finished, is malformed whatever a nested object says, never replaced by a draft earlier in the reasoning; the last complete document on the line decides and prose around it is ignored), writes `.review/<card>.pre.<cycle>.<round>.json` and `.log` next to the candidate, records the round in the card run and journals `PRE_REVIEW_DECIDED`.
