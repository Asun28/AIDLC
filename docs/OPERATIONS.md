...
`command` is argv (the prompt arrives on stdin; on Windows it runs through a shell unless `shell` is set); an empty command disables the stage. `aidlc review pre <card>` builds the prompt from `REVIEW.md`, the card (acceptance, allow_paths, non_goals, forbid, tier, diagnosis), the committed diff against the base and the run's unresolved findings with their dispositions (see Findings and dispositions), runs the command with a receipt, takes the last top-level JSON document of the whole output as the verdict (one string-aware pass over the output: outside a document only a JSON-looking brace opens one, whitespace before its first key unbounded, a brace followed by nothing but whitespace to the end is a document cut short, inside a document strings and escapes are tracked so a brace in a quoted reason opens nothing, and a document spanning lines is one document; a JSON-looking document after the decisive one that is cut short or does not parse, or an unfinished document that encloses it, makes the output malformed, whatever a nested axis says, never replaced by a draft earlier in the reasoning; prose around the document is ignored), writes `.review/<card>.pre.<cycle>.<round>.json` and `.log` next to the candidate, records the round in the card run and journals `PRE_REVIEW_DECIDED`.
