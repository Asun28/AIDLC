...
with bounded autonomy. This repository dogfoods itself: `.claude/` here is
the live copy of what `aidlc init` installs from `templates/`, and this
repo's own cards live in `specs/tasks/`. Node >= 22.18 is required; the CLI
and tests run the TypeScript sources directly (type stripping), so no build
...
Gotcha: `bin/aidlc.js` and `bin/aidlc-hook.js` load `dist/` when it exists
...
`ReleaseRunner` take a deps bag (`runner`, `git`, `gh`, `shipPath`, `now`,
...
- `aidlc.config.json` is parsed by the zod `ProjectConfig` in `src/config.ts`.
  `templates/aidlc.config.json` is the copy `aidlc init` ships; add new
  fields to both.
- `AIDLC_STATE_DIR` overrides the state directory. `AIDLC_SESSION` (else
  ...
  `AIDLC_FIX_TASK`, `AIDLC_RELEASE_APPROVAL` and `AIDLC_TARGET_ENV` feed the
  hooks and ops bindings.
