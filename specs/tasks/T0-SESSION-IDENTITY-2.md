...
acceptance:
  ...
  - 6. The changed documents promise no recovery that does not exist: `docs/OPERATIONS.md` (Sessions), `README.md` (Multi-session), `docs/ARCHITECTURE.md` (state layer) and the CHANGELOG entry state that `aidlc goal takeover` moves the goal lease only, that a card lease held by an ended session is continued by running with `AIDLC_SESSION` set to the owner session id that `aidlc card status <card>` and the PREPARE ownership stop show, and that the `aidlc card takeover` command named by that stop is a follow-up card, not a command (mirror.test.ts, prose.test.ts; the corrected sentences quoted in the ship evidence). [R6] [dod arm 1]
budget: 320
tdd: true
