...
worktree: D:\wt\AIDLC\T0-VERDICT-PROSE
allow_paths:
...
dod_exit: 0
requirements:
  - R1. WHEN the reviewer output carries a JSON-looking opener, the extractor shall read JSON from it: reaching the end of the output in any state is a document cut short, because every such state completes into a valid document (an open container closes, a dangling `,` or `:` takes a value, a key takes its value, an open string closes and drops an escape it was cut inside, and a partial `tru` or `1e` finishes its token), and the output is malformed; reaching a character JSON cannot carry there means the opener was prose, it opened no document, and the scan resumes at that character.
...
acceptance:
  ...
  - 3. The reader is exercised through `extractVerdict` at both edges: an opener refuted by a character JSON cannot carry (a key followed by `/`, an unquoted word after a closed value, an escape JSON does not define, a token that is no prefix of any value) opens nothing and the later document decides; an opener the output ends inside is a document cut short and the output is malformed, whatever parseable document precedes it, in every state the cut can land in (an object cut after a key, a separator or a colon, one cut after a nested axis, an unfinished enclosing array, a string cut in its text or inside an escape, a literal or a number cut mid-token, with or without a trailing newline). [R1] [R3] [dod arm 1]
  - 4. The walk visits each character once, so an output full of prose openers costs what its size costs and no reviewer output can stall the runner (a scaling arm in pre-review.test.ts), and the documents of one output are collected without spreading an unbounded list into one call. `docs/OPERATIONS.md` states the reading rule where it states the extraction rule today (the `command` paragraph of Pre-review (R2)), and CHANGELOG.md Unreleased carries the entry (read in review; no test arm). [dod arm 1]
depends_on: []
budget: 180
tdd: true
