...
acceptance:
  ...
  - 2. Every pre-change extraction case keeps its result, asserted in one list: an unfinished enclosing array (`[{"verdict":"pass","reasons":[]}`), a final lone `{`, a final `{` with trailing whitespace, a truncated trailing document after a complete one, a final document that does not parse, a block cut short after a nested axis, and a draft before the decisive document (pre-review.test.ts). [R3] [dod arm 1]
  - 3. The completion predicate is exercised directly through `extractVerdict` at its two edges: an opener whose remainder is not completable (a key followed by `/`, an unquoted word, a bad escape) resumes the scan and lets the later document decide, and an opener whose remainder is completable (an open string, a nested object, an array) is malformed even when a complete document follows it, because the output was cut short. [R1] [R3] [dod arm 1]
