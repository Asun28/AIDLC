...
acceptance:
  ...
  - 8. A test reads every src/ file and finds no quota word pattern outside `src/core/parse-guard.ts` except the CI log classes of `src/core/ci-policy.ts` (a non-goal of this card, exempted by name), and `src/core/parse-guard.ts` is at most 30 lines (tests/core/parse-guard.test.ts). [R5] [dod arm 1]
