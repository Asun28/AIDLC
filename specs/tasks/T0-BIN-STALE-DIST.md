...
acceptance:
  - 1. `resolveEntry({ dist, src, srcRoot })` in `bin/resolve-entry.js` returns `{ target: dist, reason: 'dist' }` when the compiled entry exists and every file under `srcRoot` is at most as new as it; `{ target: src, reason: 'no-dist' }` when the compiled entry is missing; `{ target: src, reason: 'stale-dist' }` when any file under `srcRoot` (recursively, `.ts` files) is newer than the compiled entry; a `srcRoot` that cannot be read counts as not newer (bin-entry.test.ts). [R1] [R2] [dod arm 1]
  - 2. `bin/aidlc.js` and `bin/aidlc-hook.js` both import `resolveEntry` from `./resolve-entry.js` and load its target (bin-entry.test.ts reads both files); `AIDLC_ENTRY_DEBUG=1` prints the chosen target and the reason to stderr once (bin-entry.test.ts spawns `node bin/aidlc.js --version` with the variable set). [R3] [dod arm 1]
...
## Deliverable
`bin/resolve-entry.js` exports `resolveEntry({ dist, src, srcRoot })`: the compiled entry when it exists and is at least as new as every source file, else the sources, with the reason (`dist`, `no-dist`, `stale-dist`). Both bin entries use it; `AIDLC_ENTRY_DEBUG=1` prints the choice. The docs state the rule.
