...
requirements:
  ...
  - R3. `aidlc doctor` shall print `config: ERROR <message>` with exit 1 and no stack trace only for a `ConfigError`; any other failure while it loads its context ends it with that failure's own message, as it ends every other command, and never as `config: ERROR`. The `src/cli/main.ts` change is the doctor catch alone.
...
forbid: [editing src/loop/card-runner.ts, src/core/review-policy.ts, the store, lease or journal modules (the T1-STORE-CAS line), editing src/delivery/github-ship.ts (T0-CI-RED-LOGS-BOUNDS), a main.ts hunk outside the doctor action, a read failure of aidlc.config.json reported as config: ERROR, a looser quota word rule]
