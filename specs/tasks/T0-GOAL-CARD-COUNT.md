...
dod_exit: 0
requirements:
  - R1. WHEN a request's text names more than one distinct known card id (or, when the caller passes no known card list, more than one distinct card id token), `classifyRequest` shall route it with `cardCount` `unknown` and the `arc` module, and name every such id in a reason; a request naming one known card id keeps its count, and `ref=` is the first known card id of the text, never an unknown id that comes before it.
  - R2. `GoalController.createGoal` shall take the goal's card from the text's `ref=` only when the routed `cardCount` is 1; otherwise the goal starts with no card, and the projection (`aidlc report --result cards-projected --cards ...`) names them. A request whose text names several cards gets the arc limit also when `--card` names one of them; `--card` sets the goal's cards, not the deadline.
...
acceptance:
  - 1. `tests/core/router.test.ts`: a text naming two known card ids routes with `cardCount` `unknown`, the `arc` module and a reason naming both, with or without an explicit size; a text naming one known card id, or one known and one unknown id in either order, keeps `cardCount` 1 for a T0 size with `ref=` the known id; with no known card list, two card id tokens route with `cardCount` `unknown` and a reason saying the registry was not consulted; a release or migration text and a card-amendment text that name two known ids route as card-execute and card-amendment with `cardCount` `unknown`, so the release and migration branches are never skipped. [R1] [dod arm 1]
  - 2. `tests/scenarios/goal-card-count.test.ts`: a goal from the issue #73 request (two known card ids, explicit T1) and the same request with no size start with no card and the 12 h arc deadline; a goal with `--card` one card and an explicit T1 or T2 keeps that card and gets 12 h; a goal naming one known card id with no size or an explicit T0 keeps that card and gets 3 h, and so does `--card` one card with a T1 the router inferred from impact words (only an explicit T1 or T2 forces the arc); a release goal with an explicit T1 or T2 gets 12 h while a release with no explicit size keeps the one-card 3 h, and a tighter `--limit-hours` still wins over the arc limit; the issue #73 request with `--card` one of its two cards keeps that card and gets 12 h. [R2] [R3] [dod arm 1]
...
hygiene: "Filed from issue #73. Option chosen: leave the count unknown so the projection decides (not list every named id). A request can name a card without asking to run it ('after T1-A, do T1-B'), the projection is the authority for the card list, and an unknown count selects the arc limit without guessing; each card keeps its own 3 h limit. Every acceptance item has a DoD-run test from the first candidate (docs/LESSONS.md 2026-09-26 T0-BASE-SYNC-JSDOC); run the mutation sweep before the first review. R2 edge-cases questions (answered in the candidate, not left to R3): options.cards beside several named cards is reachable (goal new naming two cards with --card one), so several named cards force the arc limit; an unknown id before a known one no longer becomes ref=; the known card list stays optional, since the UserPromptSubmit route preview runs without the registry, and without it every card id token counts, so two tokens leave the count unknown with a reason instead of a silent 3 h; the multi-card branch needs no kind guard, since a named card sets the kind to card-execute or card-amendment before release or migration is considered, which a test pins."
