...
acceptance:
  ...
  - 4. `ProjectConfig.github` (`requiredChecks` default empty, `requireVerdict` default true, `ciTimeoutMs`, `ciPollMs`) parses in both config files and reaches `GitHubShipPath` from `CardRunner` without an injected ship path; a required check absent from the returned check runs counts as pending, never as satisfied, and a required check that reports skipped or neutral fails the gate; this repository lists the four unconditional check-run names (config.test.ts, github-ship.test.ts). [R8] [dod arm 1]
