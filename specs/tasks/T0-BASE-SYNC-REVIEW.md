...
requirements:
  ...
  - R2. WHEN a candidate recorded by a successful attempt that cleared a `merge-conflict` repair, or that repairs a base-sync candidate no R3 decision has decided yet (an R2 block after the merge), (a base-sync candidate) needs R3 and the two-decision allowance is used, and `formalReview.baseSync` is configured, the SHIP gate shall issue one R3 decision on that candidate by the base-sync reviewer with its own effort policy and its own review pool, the primary staying in the pool it uses without a base-sync reviewer (a `review` directive naming it) instead of STOP/review; the prompt shall carry the delta since the last candidate R3 decided, as decision two does, and state that the base moved.
  - R3. A base-sync decision shall be taken at most once per base-sync candidate: its pass opens the ship; its block stops the card for review with the findings retained (no repair loop past the allowance); a no-verdict takes the card's single no-verdict retry; a quota hold of the base-sync reviewer waits; the invocation is recorded in the review ledger under the base-sync reviewer's name and marked as a base-sync decision.
...
acceptance:
  - 1. With both R3 decisions used and a base-sync candidate recorded, `card next` returns a `review` directive naming the base-sync reviewer; `review r3` dispatches its command with `{effort}` expanded to `medium`; a pass lets `card next` ship; the base-sync request queues in `<reviewPool>/<base-sync reviewer>` and the primary's requests stay in `<reviewPool>`; a base-sync candidate that R2 blocks is repaired into a candidate that still gets the base-sync decision (tests/scenarios/base-sync-review.test.ts). [R1] [R2] [R3] [dod arm 1]
