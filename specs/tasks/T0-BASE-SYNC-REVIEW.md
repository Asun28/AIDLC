...
acceptance:
  ...
  - 4. Each of these still stops for review past the allowance: no `formalReview.baseSync`, a candidate from a review repair, and a second base-sync decision on the same base-sync candidate; a later base-sync candidate (another conflict) gets its own decision (tests/scenarios/base-sync-review.test.ts). [R3] [R4] [dod arm 1]
