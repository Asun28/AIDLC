...
dod_exit: 0
requirements:
  - R1. WHEN the failed step is not step 1, the ship path shall place its start only at a `##[group]Run ` header that is the failed step's own on two counts - its group holds the runner's `shell: ` line, and it is the one such line in the failed step's first second that reads `##[group]<step name>` (the header a step named after its command prints) - and shall write `step unknown` otherwise (no such line or two of them, a step with a name of its own); no count of the steps that started in that second decides it, since post steps and `Complete job` start there without a header, so the `run` headers an earlier composite step printed in that second, whether it started or completed there, never place it.
