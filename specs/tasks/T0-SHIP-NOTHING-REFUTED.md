...
acceptance:
  - 1. `tests/scenarios/t0-flow.test.ts`: a card whose run has no effort episode when the ship fails with `dod-failed` gets a build directive whose narration contains the new no-refutation sentence and not "counts as a failed attempt", names attempt 1, and the following `card next` build directive names attempt 1; a card whose ship failure refutes a success keeps "The failure counts as a failed attempt on the effort ladder." [R1] [R2] [dod arm 1]
