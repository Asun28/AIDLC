---
id: T0-BIN-STALE-DIST-2
title: The bin entries load the sources when the compiled build is older than any source file, so a source checkout never runs stale code (replacement of T0-BIN-STALE-DIST after its two R3 decisions)
status: todo
branch: T0-BIN-STALE-DIST-2
worktree: D:\wt\AIDLC\T0-BIN-STALE-DIST-2
superseded_by: T0-BIN-STALE-DIST-3
allow_paths:
  - bin/aidlc.js
  - bin/aidlc-hook.js
...
## Deliverable
Replacement of T0-BIN-STALE-DIST, which stopped after its two R3 decisions on branch T0-BIN-STALE-DIST (cf7607d): decision 1 (3 findings, repaired) and decision 2 (2 findings: link tests that could be bypassed by swallowed setup errors, and a child inspection failure mistaken for an unreadable root under `withFileTypes`). This card carries the same change with those two repaired first, then a fresh R2 cycle and two R3 decisions.
...
- Assertion: the resolver tests pass for the four reasons, the linked cases (or report them skipped where the platform refuses links), the descendant failures including a child that cannot be inspected under a readable root; both bin files import the resolver; the spawn regression runs both entries over a stale and a current build; the init tests still pass.
