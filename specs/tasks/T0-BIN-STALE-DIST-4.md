---
id: T0-BIN-STALE-DIST-4
title: The bin entries load the sources when the compiled build is older than any source file, so a source checkout never runs stale code (replacement of T0-BIN-STALE-DIST-3 after its CI stop on a conflicting PR)
status: merged
branch: T0-BIN-STALE-DIST-4
worktree: D:\wt\AIDLC\T0-BIN-STALE-DIST-4
allow_paths:
  - bin/aidlc.js
  - bin/aidlc-hook.js
  ...
  - specs/tasks/T0-BIN-STALE-DIST-4.md
...
## Deliverable
Replacement of T0-BIN-STALE-DIST-3 (branch T0-BIN-STALE-DIST-3, f924d9a: R2 passed twice, R3 decision 1 blocked on two test findings, repaired, decision 2 passed). Its ship opened PR #15 while main had merged T0-SESSION-IDENTITY-3, so the PR conflicted on CHANGELOG.md, GitHub ran no workflow, the CI gate timed out with zero checks and the card stopped with reason ci. This card carries the same change with main merged (CHANGELOG resolved), then a fresh R2 cycle, two R3 decisions and the ship.
