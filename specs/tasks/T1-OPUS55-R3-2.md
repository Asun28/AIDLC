...
requirements:
  ...
  - R5. The candidate diff collected for a review shall be taken without colour and without an external diff driver, and a non-empty collected diff with no `diff --git ` section shall select `high` rather than a count of zero.
...
acceptance:
  ...
  - 7. `collectCandidateDiff` runs `git diff` with `--text --no-color --no-ext-diff` on `<base>...<candidateSha>`, so the diff the reviewer receives and the diff the level is counted from carry no colour escapes and no external-driver output under any user git configuration (`color.ui=always`, `diff.external`, `GIT_EXTERNAL_DIFF`); a non-empty collected diff with no `diff --git ` section selects `high`, never a count of zero, and a colour-escaped diff fixture is a case of it (pre-review.test.ts asserts the argv of the diff call; review-effort.test.ts the fail-closed case). [R5] [dod arm 1]
