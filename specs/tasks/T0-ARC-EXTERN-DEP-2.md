...
acceptance:
  ...
  - 6. `renderBoard` reports the same arc verdict and wave as the directive for a projection whose prerequisite merged elsewhere: it takes the resolved outcomes from its caller instead of rebuilding them from the projected cards alone, and `aidlc board` prints the one text it writes rather than a second render of its own (board.test.ts). [R3] [dod arm 1]
budget: 300
tdd: true
