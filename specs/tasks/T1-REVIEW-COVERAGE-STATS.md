...
acceptance:
  - 1. `summarizeReviews` returns per card `coverage` with `roundsRequested` (decided rounds carrying a `coverage` record), `roundsComplete` (of those, rounds with no unaccounted, conflicted or inconsistent item and no malformed entry), `unaccounted`, `conflicted` and `inconsistent` (item counts summed over the rounds) and `r3SpecFindingsAfterIncomplete` (formal findings on the spec axis whose `candidateSha` equals that of a decided R2 round with an unaccounted item and no later decided R2 round on that candidate), read off the round records and the findings only (stats.test.ts). [R11] [dod arm 1]
