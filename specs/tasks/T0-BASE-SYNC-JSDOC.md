...
dod_command: npm run typecheck && node --test tests/surface/base-sync-jsdoc.test.ts
dod_exit: 0
requirements:
...
forbid: ["any change to src/loop/card-runner.ts other than the JSDoc comment lines directly above formalPool and formalReviewerFor (exception granted by the coordinating session for this card only; card-runner.ts is otherwise owned by T1-PARSE-GUARD)", "any change to src/core/types.ts other than the JSDoc of CandidateInfo.baseSync", a changed code token in either file, a new no-verdict retry for the base-sync decision]
non_goals: [issue #65 item 3 (ruled: no change, see hygiene), the inline comments inside the method bodies, docs/OPERATIONS.md]
...
```powershell
npm run typecheck && node --test tests/surface/base-sync-jsdoc.test.ts
```
