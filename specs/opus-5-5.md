...
# Spec: Run R3 at an effort that follows the diff, and bring the Claude defaults and prompts in line with the Opus 5.5 guides
...
record. Each reviewer uses its own `effort` object; a reviewer whose argv
has no `{effort}` is unaffected.
...
- The author sessions and the R3 reviewer are both Claude Opus 5.5. The
  ...
  consulted by the command path. Accepted by the engineer; Codex stays
  the primary, so this holds only while Codex is on a quota hold.
...
- 3. `aidlc.config.json` runs R3 on Codex gpt-6-sol and falls back to Claude Opus 5.5, both with `{effort}`. [R4]
