...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  review
    ...
    .action((cardId: string, o: { goal?: string }) => {
      const c = ctx(g());
      const { goalRec, parsed, run } = cardCtx(c, cardId, o.goal);
      const r = runnerFor(c).formalReview(goalRec, parsed.card, run);
      c.controller.writeBoard(goalRec);
      const summary = { outcome: r.classified.outcome, mergeBlocking: r.classified.mergeBlocking, runStatus: r.classified.runStatus, decisions: r.run.review.substantiveDecisions, blocks: r.run.review.substantiveBlocks, reviewer: c.config.formalReview.reviewer, durationMs: r.receipt.durationMs, reasons: r.classified.reasons, verdictRef: r.verdictRef, logRef: r.logRef, state: r.run.state };
