...
const PORCELAIN = [
  ...
  '',
  'worktree C:/wt/T1-FOO',
  'HEAD 2222222222222222222222222222222222222222',
  'branch refs/heads/T1-FOO',
  'locked',
  '',
  'worktree C:/wt/scratch',
...
describe('probes/git (evidence probes with a scripted runner)', () => {
  it('parses worktree porcelain including branch, detached, locked and prunable', () => {
    ...
    const entries = probe.worktrees('C:/repo');
    assert.equal(entries.length, 3);
    assert.equal(entries[0]!.path, path.resolve('C:/repo'));
  ...
  it('findWorktree matches exact branch and canonical path; a path mismatch is reported, not adopted', () => {
    const probe = new GitProbe(scriptedRunner({ 'git worktree list --porcelain': { stdout: PORCELAIN } }));
    const hit = probe.findWorktree('C:/repo', 'T1-FOO', 'C:\\wt\\T1-FOO');
    assert.equal(hit.mismatch, undefined);
    assert.equal(hit.entry?.branch, 'T1-FOO');
    const mismatch = probe.findWorktree('C:/repo', 'T1-FOO', 'C:/elsewhere/T1-FOO');
    assert.ok(mismatch.entry);
    assert.match(mismatch.mismatch ?? '', /expected .*elsewhere/);
    assert.deepEqual(probe.findWorktree('C:/repo', 'T9-NONE'), {});
    // the same branch in two worktrees is ambiguous
    const dup = new GitProbe(scriptedRunner({ 'git worktree list --porcelain': { stdout: PORCELAIN + 'worktree C:/wt/dup\nbranch refs/heads/T1-FOO\n' } }));
    assert.match(dup.findWorktree('C:/repo', 'T1-FOO').mismatch ?? '', /checked out in 2 worktrees/);
  });
  ...
  it('a failing git command throws GitProbeError with the receipt (errors are not empty results)', () => {
    ...
    assert.throws(() => probe.head('C:/nowhere'), (e: unknown) => e instanceof GitProbeError && /not a git repository/.test(e.message) && e.receipt.exitCode === 128);
    assert.equal(probe.currentBranch('C:/nowhere'), undefined);
  });
  ...
  it('candidate digest binds HEAD to the working-tree status and input manifest', () => {
    ...
    const clean = probe.candidate('C:/wt');
    assert.equal(clean.sha, 'abcdef');
    assert.equal(clean.dirty, false);
    assert.deepEqual(clean.untracked, []);
    status = ' M src/a.ts\n?? tests/new.test.ts\n';
    const dirty = probe.candidate('C:/wt');
    assert.equal(dirty.dirty, true);
    assert.deepEqual(dirty.untracked, ['tests/new.test.ts']);
    assert.notEqual(dirty.digest, clean.digest);
    status = '';
    assert.equal(probe.candidate('C:/wt').digest, clean.digest);
    assert.notEqual(probe.candidate('C:/wt', ['fixtures/golden.json@1']).digest, clean.digest);
  });
  ...
  it('resolveBase prefers the remote ref, then local, fully qualified and verified as a commit', () => {
    ...
    assert.deepEqual(both.resolveBase('C:/repo', 'main'), { ref: 'refs/remotes/origin/main', oid: 'remote-oid' });
    assert.deepEqual(both.resolveBase('C:/repo', 'origin/main'), { ref: 'refs/remotes/origin/main', oid: 'remote-oid' });
    assert.deepEqual(both.resolveBase('C:/repo', 'main', true), { ref: 'refs/heads/main', oid: 'local-oid' });
    ...
    assert.deepEqual(localOnly.resolveBase('C:/repo', 'main'), { ref: 'refs/heads/main', oid: 'local-oid' });
    assert.equal(localOnly.resolveBase('C:/repo', 'origin/main'), undefined);
    const none = new GitProbe(scriptedRunner({}));
    assert.equal(none.resolveBase('C:/repo', 'main'), undefined);
  });
  ...
  it('divergence parses left-right counts; contains/numstat/changedPaths read their commands', () => {
    const probe = new GitProbe(
      scriptedRunner({
        ...
        'git rev-parse --git-common-dir': { stdout: 'C:/repo/.git\n' },
        'git fetch --quiet --no-tags origin +refs/heads/main:refs/remotes/origin/main': { exitCode: 0 },
      }),
    );
    assert.deepEqual(probe.divergence('C:/wt', 'refs/remotes/origin/main'), { behind: 2, ahead: 3 });
    assert.equal(probe.contains('C:/wt', 'refs/remotes/origin/main', 'deadbeef'), true);
    assert.equal(probe.contains('C:/wt', 'refs/remotes/origin/main', 'cafebabe'), false);
    assert.deepEqual(probe.changedPaths('C:/wt', 'base'), ['src/a.ts', 'docs/b.md']);
    assert.deepEqual(probe.numstat('C:/wt', 'base'), { added: 13, deleted: 2, files: 3 });
    assert.equal(probe.commonDir('C:/wt'), path.resolve('C:/repo/.git'));
    assert.equal(probe.fetchBase('C:/wt', 'origin/main').exitCode, 0);
  });
});
